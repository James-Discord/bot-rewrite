    ...
    if (userData.get(message.author.id) == null) {
        ...
        collector.on('collect', messagecollected => {
            ...
            setTimeout(async () => {
                ...
                if (!consoleUser) {
                    channel.send('There are no users with that account! \nRemoving channel!')
                    setTimeout(() => {
                        channel.delete();
                    }, 5000)
                } else {
                    ...
                    transport.sendMail(emailmessage, function (err, info) {
                        if (err) {
                            ...
                            console.log(info);
                            channel.send('You will now get a verification email from us with a code - please type that in here! You have 2 minutes!')
